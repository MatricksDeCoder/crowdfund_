...
describe("Project", function () {
  ...
  beforeEach(async function () {
    ...
    await projectFactory
      .connect(creator)
      .createProject(goal1.toString(), badgeName, badgeSymbol);
  ...
  describe("deployment", async function () {
    it("should revert if goal is less than 0.01 ETH", async function () {
      await expect(
        projectFactory
          .connect(creator)
          .createProject(goal2.toString(), badgeName, badgeSymbol)
      ).to.be.revertedWith("goal too low");
    });
  ...
  describe("contribute()", async function () {
    describe("success", async function () {
      beforeEach(async function () {
        await project
          ...
          .contribute({ value: contribution1.toString() });
        ...
        await project
          .connect(contributor1)
          .contribute({ value: contribution2.toString() });
        await project
      ...
      it("tracks contributors amounts", async function () {
        ...
        expect(result.toString()).to.equal(
          (contribution1 * 2 + contribution2).toString()
        );
      ...
      it("tracks total contributions", async function () {
        result = await project.contributed();
        expect(result.toString()).to.equal(
          (contribution1 * 2 + contribution2 * 2 + contribution1).toString()
        );
      });
    ...
    describe("failure", async function () {
      it("should revert if contribution is less than 0.01 ETH", async function () {
        await expect(
          project.connect(creator).contribute({ value: contrFail.toString() })
        ).to.be.revertedWith("minimum contribition 0.01 ETH");
      });

      it("should revert if project goal reached", async function () {
        await project
          .connect(contributor2)
          .contribute({ value: goal1.toString() });
        await expect(
          project
            .connect(creator)
            .contribute({ value: contribution1.toString() })
  ...
  describe("cancel", async function () {
    it("should revert if not called by creator", async function () {
      await expect(project.connect(contributor1).cancel()).to.be.revertedWith(
        "not project creator"
      );
    });
    ...
    it("should revert contribution to cancelled project", async function () {
      await project.connect(creator).cancel();
      await expect(
        project.connect(creator).contribute({ value: contribution1.toString() })
      ).to.be.revertedWith("project is cancelled");
    });
    ...
    it("cannot cancel project that passed closeDate", async function () {
      ...
      await provider.send("evm_increaseTime", [thirtyDays]);
      await provider.send("evm_mine");
      await expect(project.connect(creator).cancel()).to.be.revertedWith(
        "project is closed"
      );
    });
  });
  ...
  describe("contributorWithdraw()", async function () {
    ...
    describe("success", async function () {
      it("allows withdraws with right amounts if project is cancelled", async function () {
        ...
        let reconcile =
          (totalContributed - Number(ethAfter) + Number(ethBefore)) / 10 ** 18; //should be close to 0 in ETH
    ...
    describe("failure", async function () {
      it("should revert if project not cancelled", async function () {
        await expect(
          project.connect(creator).contributorWithdraw()
        ).to.be.revertedWith("only project failed");
      });
      ...
      it("should revert if contributor has withdrawn already", async function () {
        ...
        await project.connect(contributor1).contributorWithdraw();
        await expect(
          project.connect(contributor1).contributorWithdraw()
        ).to.be.revertedWith("already withdrew");
      });
    });
  });
  ...
  describe("creatorWithdraw()", async function () {
    ...
    describe("success", async function () {
      ...
      it("allows any withdraws from creator and emits event", async function () {
        await project
          .connect(contributor1)
          .contribute({ value: (contribution1 * 4).toString() }); //ensure target reached
        let reconcile;
        ...
        const thirty1Days = 60 * 60 * 24 * 31;
        await provider.send("evm_increaseTime", [thirty1Days]);
        await provider.send("evm_mine");
        // withdraw part amount 4 ETH
        await project
          .connect(creator)
          .creatorWithdraw(withdrawAmount.toString());
        ...
        reconcile =
          (withdrawAmount - Number(ethAfter) + Number(ethBefore)) / 10 ** 18; //should be close to 0 in ETH
        reconcile = Math.round(reconcile, 4);
        expect(reconcile).to.equal(0);
        ...
        const id = await project.projectId();
        await expect(
          project.connect(creator).creatorWithdraw(withdrawAmount.toString())
        )
    ...
    describe("failure", async function () {
      it("should revert if project cancelled", async function () {
        await project.connect(creator).cancel();
        await expect(
          project.connect(creator).creatorWithdraw(contribution1.toString())
        ).to.be.revertedWith(
      ...
      it("should revert if project goal reached", async function () {
        await project
          .connect(contributor1)
          .contribute({ value: contribution1.toString() }); //previously 4 times from beforeEach
        let contributed = await project.contributed();
        console.log(contributed);
        await expect(
          project.connect(creator).creatorWithdraw(contributed.toString())
        ).to.be.revertedWith(
          "project not successful"
        );
      });
